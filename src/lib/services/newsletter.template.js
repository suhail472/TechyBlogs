/**
 * Email-safe HTML Newsletter Compiler for TechyBlogs
...
export function buildNewsletterHTML({
  ...
  const storiesHtml = featuredStories
    .map((story, idx) => {
      const storyUrl = `${story.url || `https://techyblogs.com/blogs/${story.slug || ''}`}?utm_source=newsletter&utm_medium=email&utm_campaign=${encodeURIComponent(
        utmCampaign
      )}`;
...
<head>
  ...
  <title>${campaignTitle || 'TechyBlogs Morning Briefing'}</title>
  <!--[if mso]>
  <style type="text/css">
...
<body style="margin: 0; padding: 0; background-color: #f4f4f5; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
  ...
  <div style="display: none; font-size: 1px; color: #f4f4f5; line-height: 1px; max-height: 0px; max-width: 0px; opacity: 0; overflow: hidden;">
    ${previewText || 'Today’s top editorial stories and analysis from TechyBlogs.'}
  </div>
          ...
          <tr>
            <td style="padding: 24px 32px; background-color: #fafafa; border-top: 1px solid #e4e4e7; text-align: center; font-size: 12px; color: #71717a; line-height: 1.5;">
              <p style="margin: 0 0 10px 0;">
                You are receiving this newsletter because you subscribed to TechyBlogs ${edition} Briefings.
              </p>
              ...
              <p style="margin: 0; font-size: 11px; color: #a1a1aa;">
                &copy; ${new Date().getFullYear()} TechyBlogs Digital Publication. All rights reserved.
              </p>
            </td>
          </tr>
