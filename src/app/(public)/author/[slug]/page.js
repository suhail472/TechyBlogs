...
async function getAuthor(slug) {
  try {
    ...
    const author = await Admin.findOne({ $or: [{ slug }, { username: slug }] })
      .select('name slug username avatar bio expertise website socialLinks role')
      .lean();
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: author.bio || `Read the latest work from ${author.name} on TeachyBlogs.`,
    ...
    <main className="max-w-7xl mx-auto px-6 md:px-10 pt-32 pb-16">
      ...
      <header className="max-w-3xl pb-9 border-b-4 border-zinc-950 dark:border-white flex gap-6 items-start">
        {author.avatar ? (
          ...
          <div className="w-20 h-20 rounded-full bg-red-700 text-white grid place-items-center font-display text-2xl font-black">
            {author.name.slice(0, 1)}
          </div>
        )}
        <div>
          <p className="text-[10px] uppercase tracking-[0.2em] font-black text-red-700 dark:text-red-400">
            {author.role || 'Author'}
          </p>
          <h1 className="font-display text-4xl font-black mt-2">{author.name}</h1>
          {author.bio && <p className="mt-3 text-zinc-600 dark:text-zinc-300 leading-relaxed">{author.bio}</p>}
          {author.expertise?.length > 0 && (
            <p className="mt-3 text-xs font-bold text-zinc-500">Covers: {author.expertise.join(' · ')}</p>
          )}
          {author.website && (
            <a
              href={author.website}
              ...
              target="_blank"
              className="inline-flex items-center gap-1.5 mt-4 text-xs uppercase tracking-wider font-bold text-red-700 dark:text-red-400"
            >
      ...
      <section className="pt-8">
        <h2 className="font-display text-2xl font-black mb-6">Latest from {author.name}</h2>
        {posts.length ? (
      ...
      <Link
        href="/blogs"
        className="inline-flex items-center gap-1.5 mt-10 text-xs uppercase tracking-wider font-bold text-red-700 dark:text-red-400"
      >
