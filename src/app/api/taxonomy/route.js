...
export async function POST(req) {
  try {
    ...
    if (!canManageTaxonomy(user)) return NextResponse.json({ success: false, message: 'Editor permission required' }, { status: 403 });
    const body = await req.json();
    const slug = String(body.slug || body.name || '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    const item = await Taxonomy.create({ ...body, slug });
