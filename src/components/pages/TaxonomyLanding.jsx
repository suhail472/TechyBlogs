    ...
    <main className="max-w-7xl mx-auto px-6 md:px-10 pt-32 pb-16">
      ...
      {!hero ? (
        ...
        <>
          ...
          <section className="pt-8"><h2 className="font-display text-2xl font-black mb-6">Latest {title}</h2><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{posts.slice(1).map((post) => <BlogCard key={String(post._id)} blog={post} />)}</div></section>
        </>
      )}
    </main>
  );
